...
| 序号 | 模块 | 文件 | 内容概述 |
|------|------|------|----------|
| 1 | 正则基础 | [01-正则基础.md](./01-正则基础.md) | 创建方式、元字符、修饰符、边界符、字符集合 |
| 2 | 量词与分组 | [02-量词与分组.md](./02-量词与分组.md) | 量词、贪婪与非贪婪、分组、反向引用、断言 |
| 3 | 正则方法 | [03-正则方法.md](./03-正则方法.md) | test、exec、match、replace、split 等方法 |
| 4 | 常用正则模式 | [04-常用正则模式.md](./04-常用正则模式.md) | 手机号、邮箱、URL、日期等常用验证模式 |
...
// ✅ 预编译正则表达式
const regex = /^1[3-9]\d{9}$/; // 编译一次，多次使用
